    ...
    <>
      ...
      >
        {user?(
         ...
         <Avatar className="User Avatar" alt={user.result.name}src={user.result.imageUrl}>{user.result.name.charAt(0)}</Avatar>
         <div className="mt-5">
         <Typography className="Icons" variant="h6">{user.result.name}</Typography>
          </div>
          <div className="mt-5">
          <Button   variant="outlined"  className="Logaut" color="primary" onClick={Logout}><b className="Icons">Logout</b></Button>
           </div>
         </div>
        ):( 
